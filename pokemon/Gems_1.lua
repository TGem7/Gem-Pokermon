...
local mega_sceptile={
  ...
  loc_vars = function(self, info_queue, center)
    ...
    local card_vars = {center.ability.extra.money_mod, center.ability.extra.money_earned,
                       math.min(14, find_other_poke_or_energy_type(center, "Grass") * center.ability.extra.money_mod + center.ability.extra.money_mod)}
    add_target_cards_to_vars(card_vars, center.ability.extra.targets)
    return {vars = card_vars}
  end,
  rarity = "poke_mega",
...
local joltik = {
  ...
  poke_custom_prefix = "Gem",
  pos = {x = 3, y = 7},
  config = {extra = {hazard_ratio = 10, to_gold = 1, reset_gold = 1, rounds = 3, all_hazard = {}, hazard_to_gold = {}}},
  loc_vars = function(self, info_queue, card)
    type_tooltip(self, info_queue, card)
    ...
    return {vars = {to_add, abbr.hazard_ratio, abbr.reset_gold, abbr.rounds}}
  end,
  rarity = 2,
  cost = 6,
  stage = "Basic",
  ...
  calculate = function(self, card, context)
    if context.setting_blind then
      ...
      card.ability.extra.hazard_to_gold = {}
    ...
    if context.end_of_round and context.cardarea == G.hand then
      if #card.ability.extra.all_hazard == 0 and #card.ability.extra.hazard_to_gold == 0 then
        ...
        -- get 3 of them
        if #card.ability.extra.all_hazard <= card.ability.extra.reset_gold then
          --print("3 or less")
          card.ability.extra.hazard_to_gold = card.ability.extra.all_hazard
        else 
          for i = 1,card.ability.extra.reset_gold do
            -- get one to hazard_to_gold and remove it from pos
            local tmp_hazard = math.random(#card.ability.extra.all_hazard)
            card.ability.extra.hazard_to_gold[#card.ability.extra.hazard_to_gold+1]=card.ability.extra.all_hazard[tmp_hazard]
            table.remove(card.ability.extra.all_hazard,tmp_hazard)
          end
        end
      end
      -- turn them into gold
      if context.individual and not context.blueprint and table.contains(card.ability.extra.hazard_to_gold,context.other_card) then
        context.other_card:set_ability(G.P_CENTERS.m_gold,nil,true)
        return {
          message = localize("Gem_gold"),
          colour = G.C.GREY
        }
      end
    end
    return level_evo(self, card, context, "j_Gem_galvantula")
  end,
}
-- Galvantula
local galvantula = {
...
list = {cubone, marowak, alolan_marowak, treecko, grovyle, sceptile, mega_sceptile, torchic, combusken, blaziken, mega_blaziken, mudkip, marshtomp, swampert, mega_swampert, mawile, mega_mawile, meditite, medicham, mega_medicham, joltik, galvantula, pincurchin, indeedee_f, indeedee_m, capsakid, scovillain}
